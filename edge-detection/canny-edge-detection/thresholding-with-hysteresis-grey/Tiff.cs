...
namespace cannyEdgeDetection
{
    public class tiff
    {
        ...
        public void process()
        {
            ...
            Console.WriteLine("About to perform thresholding-with-hysterysis on file "+uri[constants.DESTINATION_FILE].ToString()+"sobel_horizontal_vertical_squared_summed_sqrted.tiff, which holds the magnitudes and file "+uri[constants.DESTINATION_FILE].ToString()+"sobel_horizontal_vertical_slope.tiff, which holds the directions or slopes and file "+uri[constants.DESTINATION_FILE].ToString()+"-non-maximum-suppressed.tiff which has the edges...");
            sundry.thresholding_with_hysterysis(ref pixels_double_squared_horizontal_vertical_sobel_squared_summed_sqrted, ref pixels_double_quotient, ref pixels, bitmapSource.PixelHeight, bitmapSource.PixelWidth, sizeOfSinglePixelInNumberOfBytes);
            Console.WriteLine("Done performing thresholding-with-hysterysis...");
            sundry.save2tiff(uri[constants.DESTINATION_FILE].ToString()+"-non-maximum-suppressed-with-hysterysis.tiff", ref pixels, bitmapSource.PixelWidth, bitmapSource.PixelHeight, bitmapSource.DpiX, bitmapSource.DpiY);
            Console.WriteLine("Saved to file "+uri[constants.DESTINATION_FILE].ToString()+"-non-maximum-suppressed-with-hysterysis.tiff");
                                                
            Console.WriteLine("**\n*\nDone doing " + uri[constants.DESTINATION_FILE].ToString());
        }       
    }
}