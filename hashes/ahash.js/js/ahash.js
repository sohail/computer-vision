...
function reduce_size(image, settings) 
{
   ...
   var pixels_per_sample = {horizontally: Math.floor((image.width - 2 * settings.bleed) / settings.columns), vertically: Math.floor((image.height - 2 * settings.bleed) / settings.rows)};
   ...
   for (var r = 0; r < settings.rows; r++) 
   {
	   ...
	   samples[r][c] = {red: ~~(rgb.red / pixel_count), green: ~~(rgb.green / pixel_count), blue: ~~(rgb.blue / pixel_count)};
       }		   
   }         
   return samples;
}
